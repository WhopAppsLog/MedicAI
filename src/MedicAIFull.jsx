...
export default function MedicAI() {
  ...
  const sendToAI = async (input) => {
    ...
    setAiResponseText('Simulated AI response: ' + input.slice(0, 120));
    setIsLoading(false);
  };
    ...
    <div className="min-h-screen flex items-start justify-center p-6 bg-slate-50">
      <div className="w-full max-w-2xl">
        <div className="text-center mb-6">
          <h1 className="text-3xl font-bold">MedicAI</h1>
          <p className="text-sm text-gray-600">Light-mode friendly UI — improved contrast</p>
        </div>

        <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow"> 
          <h2 className="font-semibold mb-2">AI Response</h2>
          <div className="min-h-[80px] p-4 bg-gray-50 rounded-md">
            {isLoading ? <p className="text-gray-500">Analyzing...</p> : (aiResponseText ? <p>{aiResponseText}</p> : <p className="text-gray-400">Your AI assistant will respond here.</p>)}
          </div>

          <form onSubmit={handleSubmit} className="mt-4">
            <textarea value={transcript} onChange={(e) => setTranscript(e.target.value)} placeholder="Describe symptoms or ask a question..." className="w-full p-3 border rounded-md bg-white" rows={4} />
            <div className="flex items-center gap-2 mt-3">
              <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-md">Analyze</button>
              <button type="button" onClick={startRecording} className="px-3 py-2 bg-gray-100 rounded-md">Start Mic</button>
              <button type="button" onClick={stopRecording} className="px-3 py-2 bg-gray-100 rounded-md">Stop Mic</button>
              <button type="button" onClick={startCamera} className="px-3 py-2 bg-gray-100 rounded-md">Open Camera</button>
          ...
          {showCamera && (
            <div className="mt-4 bg-gray-50 p-3 rounded-md">
              ...
              <div className="flex gap-2 mt-2">
                <button onClick={capturePhoto} className="px-3 py-2 bg-green-500 text-white rounded-md">Capture</button>
                <button onClick={stopCamera} className="px-3 py-2 bg-red-500 text-white rounded-md">Close</button>
              </div>
            </div>
          )}
          ...
          {capturedImg && (
            <div className="mt-4">
              <img src={capturedImg} alt="captured" className="rounded-md max-h-64" />
            </div>
          )}
        </div>
      </div>
  ...
  const aiResponse = (input, prevMsgs = []) => {
    ...
    if (prevMsgs.length > 0) {
      ...
      return (
        <div className={`min-h-screen ${data.darkMode ? 'bg-gray-900' : 'bg-slate-50'}`}>
            ...
            <div className="space-y-4 pb-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                {(data.layout || []).map(featureId => {
                  ...
                  const buttonClass = `${data.darkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-100'} rounded-3xl p-6 shadow-lg hover:shadow-xl transition-shadow`;
                  const textClass = `text-sm font-semibold ${data.darkMode ? 'text-gray-100' : 'text-gray-900'}`;
